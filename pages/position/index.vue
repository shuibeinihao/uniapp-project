<script setup lang="ts">
  ...
  const {
    ...
    url: '/mobile/enterprisepost/list',
    method: 'get',
    manual: true,
    defaultParams: {
      postStatus: 'E'
  ...
  const handleRoutePost = (postInfo) => {
    let url = `/pages/company/index?postId=${postInfo.postId}&tabValue=3&offerPrice=${postInfo.offerPrice}&enterpriseId=${postInfo.enterpriseId}&enterpriseName=${encodeURIComponent(postInfo.enterpriseName || '')}`;
    if (referrerId.value) {
  ...
  onLoad(() => {
    app.getUserInfo().then(res => {
      ...
      page[page.length - 1].onShareAppMessage = () => {
        let postId = curRow.value.postId;
        const url = `workType=${query.value.workType}&postLight=${query.value.postLight}&workCity=${query.value.workCity}&v=${searchValue.value}&offerPrice=${query.value.offerPrice}&postId=${postId}&referrerId=${res.customerId}`;
        console.log('分享地址：', `/pages/position/search?${url}`)
        return {
          title: decodeURIComponent(curRow.value.enterpriseName || '嘉位招聘 说到做到'),
          path: `/pages/position/search?${url}`
        }
  ...
  const rowPk = (row) => {
    console.log('row', row)
    const index = pkList.value.findIndex(item => item.postId === row.postId)
    if (index > -1) {
      return;
    }
  ...
  <view class="page">
    <view class="header">
      <view class="search-bar-container" style="display: flex;align-items: center;padding-left: 20rpx;">
        ...
        <view style="flex: 1">
          <uni-search-bar :focus="true" v-model="searchValue" cancelButton="none" @confirm="handleSearch(searchValue)" @clear="handleSearch('')">
          </uni-search-bar>
        </view>
      </view>
    ...
    <view v-if="state.rows?.length>0" class="list">
      <view v-for="item in state.rows" :key="item.postId" class="list-item" @click="()=> handleRoutePost(item)">
        ...
        <view class="list-item-gaps">
          ...
          <text class="list-item-gap"
            v-if="item.workType">{{switchState?.data?.msg === '1' && item.workType == 1 ? '包装工' : getRowsName(item.workType, labelState.data?.enterprise_post_info_work_type)}}</text>
